...
function FormProduto() {
    ...
    console.log("Produto antes do envio:", JSON.stringify(produto, null, 2));


    async function buscarProdutoPorId(id: string) {
        try {
            const produtoEncontrado: Produto = await buscarLogado(`/produtos/${id}`, setProduto, {
                headers: { Authorization: token }
            });
    
            if (produtoEncontrado.categoria?.id) {
                const categoriaEncontrada: Categoria = await buscarLogado(`/categorias/${produtoEncontrado.categoria.id}`, setCategoria, {
                    headers: { Authorization: token }
                });
    ...
    useEffect(() => {
        if (categoria.id) { // Só atualiza se houver ID válido
            setProduto((prevProduto) => ({
                ...prevProduto,
                categoria: {
                    id: categoria.id,
                    nome: categoria.nome || "Categoria Padrão",
                    foto: categoria.foto || "",
                },
            }));
        }
