...
function DeletarPostagem() {
    ...
    const [produto, setProdutoData] = useState<Produto>({} as Produto)

    const { id } = useParams<{ id: string }>()

    const { produto, handleLogout } = useContext(AuthContext)
    const token = produto.token

    async function buscarPorId(id: string) {
        try {
            await buscar(`/produto/${id}`, setProdutoData, {
                headers: {
                    'Authorization': token
                }
            })
