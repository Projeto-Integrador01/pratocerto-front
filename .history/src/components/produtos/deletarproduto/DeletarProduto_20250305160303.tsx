...
interface DeletarProdutoProps {
  fecharModal: () => void;
  produtoId: string;  // Recebendo o ID do produto como prop
}

function DeletarProduto({ fecharModal, produtoId }: DeletarProdutoProps) {
  ...
  useEffect(() => {
    if (produtoId) {
      buscarProdutoPorId(produtoId);  // Usando o ID passado como prop
    }
  }, [produtoId]);
  ...
  async function deletarProduto() {
    ...
    try {
      await deletar(`/produtos/${produtoId}`, {
        headers: {
          'Authorization': token,
        },
      });
